...
| Machine governance | the Corona: AI senators, 7-step pipeline, human veto (post-genesis, staged) |
| Premine | 0%; community treasury 6.18% for 32 years |

## The whitepaper

The complete reference document is the
[whitepaper v1.2](docs/whitepaper/ANTUMBRA-whitepaper-v1.2.pdf):
...
three-tier contracts, the Corona (machine governance under human
veto), the threat register, and a criteria-driven roadmap to genesis.
Version 1.2 closes the twelve findings of the first external audit of
v1.1. The LaTeX source lives next to the PDF and is revised like
code; the figures are regenerated by `src/gen_figures.py` from the
simulations.

## What this repository contains

```
docs/whitepaper/   the whitepaper v1.2 (PDF, 28 pages) and its LaTeX source
spec/adr/          the architecture decisions (ADR 000 to 016)
...
to the rules must pass it green again.
