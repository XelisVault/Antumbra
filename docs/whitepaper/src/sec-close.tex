...
requirement to the whole assembly, before an independent review takes it
...
transaction fingerprinting by authority shape & medium & uniform sixty-four-byte authority field, same derivation for human and agent spends (v1.3) & closed structurally; the test is in the vector set \\
...
right one: can all of that be coded with zero faults? The honest
answer begins with an admission: nobody can promise it, and
...
method working on the method: the first independent review of version 1.1
...
unspecified bootstrap of the Ring. Version 1.2 closes the twelve:
the numbers were corrected, the missing mechanisms were specified
(the Genesis Ring ratchet, the identity firewall, the precise
revocation of a Cipher, the capacity budget), the overclaims were cut
(the ring of sixteen now equals Monero's, it does not triple anything;
the golden ratio is now named as an identity, not an argument), and
the simulation now sweeps twelve seeds. The register of what an
outside eye catches that the author's eye does not is the whole
argument of layer five below, and this paper has now been through it
once for real.
...
of the protocol will be treated that way before implementation, with
numeric invariants, a seed sweep and a regression run over three attack
sets.
...
eyes, two reading conventions. Fifth layer: independent review of the
...
genesis, and the independent review called it what it was: a schedule for a
...
for a team of at least three full-time engineers, but the binding
term is the exit criterion of each phase, and genesis happens when
every criterion is green, not when a Gantt chart says so. The estimate
the paper stands behind is twenty-four months of full-time work to the
...
step. The base is an assembly of proven foundations, the
...
Kleos, the Ring, the Lumen, Cipher perimeters, Mandates, the Corona,
is
deterministic, testable, auditable code, with no exotic primitive
before independent review. Every
...
4, identities and Lumen & M14-M18 & Embers with sponsorship, Ciphers with warrants and fallback Mandates, bounded viewing keys, independent review of the differential & audit without a critical flaw \\
...
The machine layers above the settlement core follow their own
criteria-driven tracks, deliberately decoupled from the settlement
calendar: the chain must be born solvent and private even if every
machine layer stays dark, and every machine layer must be born testable
even if the chain is late. The binding order is the dependency order,
and every track is a published NO-GO until its criterion is green.

\begin{table}[htbp]
\centering
\small
\caption{The machine-economy tracks. Settlement genesis needs P0 to P3
only; the rest may light on testnet or later, and the network never
depends on an unlit track.}
\label{tab:machinetracks}
\begin{tabularx}{\textwidth}{@{}L{3.4cm}YL{5.6cm}@{}}
\toprule
Track & What lands & Exit criterion \\
\midrule
P0, the node & \texttt{antumbra-node} assembled, honest devnet status, minimum invariants, a day of DAG & twenty-four hours of blocks with conservation and tip invariants held \\
P1, spec-is-tests & genesis Ember table, dual Kleos vectors, the agent-farm simulation in CI & every equation of v1.3 cross-validated, bit for bit \\
P2, Thymus toy & invariants, canaries, alerts, freeze, bounty accounting, MTTD metrics & the nine invariants replay by a Sentinel, not only by a node \\
P3, machine accounts & Warrants, uniform encoding, Mandate grammar v0, receipts, streams, batches & the anonymity-set split closed, measured, in the vector set \\
P4, JobBoard & jobs, classes C0--C2, pay-for-result, contest window & scripts earn test ATU end to end on testnet \\
P5, forge & signed namespaces, policy-as-code rails, Arena, \texttt{merge\_ok} for rail B & first tooling merge by agents, rolled back cleanly once \\
P6, rail A & the registry, Senate silence, bounded changes & first parameter adjusted, inspected after the fact \\
P7, Senate and debate & ArgNode, dampened tally, quorum floors & a hostile red-team review of the tally itself \\
P8, rail C canary & one consensus class through the whole machine & one activation with a rehearsed, real rollback \\
P9, the Vault & FROST, TEEs, verifiable inference & unlit by design until audited code exists; mainnet launches without it \\
\bottomrule
\end{tabularx}
\end{table}
...
criteria-driven work, every step verifiable by a third party, none of
it depending on a miracle of cryptography, and a calendar that bends to
the evidence instead of the evidence bending to the calendar.
...
stage criteria are met: the Corona v2, a bounded class of changes that
deploy themselves under a diverse predicate, a human gate and an
immunity organ, the network pruning its own orchard while the humans
keep the veto and the fruit. The specification
