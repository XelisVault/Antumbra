...
Revocation, stated precisely because the first version of this paper
was rightly challenged on it: revoking a Cipher does not, and cannot,
...
opens to the sponsor forty-eight hours after revocation, so a fired
agent cannot sit on the company's money. The honest trade follows, and
it is a product decision, not an oversight: an agent's transactions are
...
Humans receive full unlinkability; agents receive a career with a
name. An agent's wallet is not a bottomless
...
the same run. The first pass, with the rules of the
second version, did what no re-reading would have done: it showed the
attack succeed. Twenty complicit sponsors, registering two fake
profiles per year each with impeccable behavior and an unlimited
witness budget, grew a farm of five hundred eighty profiles that
crossed the candidacy threshold at year eight
and a half, before the honest network, and took the fifty-five seats at year
...
maximum attacks, same sixteen years: zero fake candidates,
...
approaching a seat. The first independent review of this paper asked the
right question about those numbers, and the answer belongs here: one
seed is a demonstration. The engine now sweeps twelve seeds, the same
two passes on each, and Table~\ref{tab:simres} reports the worst case
across the sweep, not the canonical draw. The v2 flaw reproduces on
every seed, which is what makes it structural; the wall holds on every
seed, which is what makes it a rule.
...
\label{tab:simparams}
\begin{tabularx}{\textwidth}{@{}L{5.2cm}L{4.2cm}Y@{}}
\toprule
...
is not modeled, and against it the primary defense is R1's wall of
time, which is a design argument, not a simulated one. The whale
plateau at 30 is likewise not a tautology dressed as a result: Deed and
Tenure are non-purchasable by construction, and what the whale run
verifies is the third channel, that Echo, the only monetizable layer,
cannot convert capital into candidacy once the budget, the cap and the
weighting hold. What the simulation proves is narrow and valuable: the
rules compose deterministically, the invariants hold at every era, and
the known attack class fails on every seed tried. One honest precision
to finish: Tenure makes
